...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: any = Depends(oauth2.get_current_user)):
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: any = Depends(oauth2.get_current_user)):
...
def update_post(id: int, post: schemas.PostCreate, db: Session = Depends(get_db), current_user: any = Depends(oauth2.get_current_user)):
    post_query = db.query(models.Post).filter_by(id=id)
    if post_query.first() == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"post with id: {id} does not exist")
